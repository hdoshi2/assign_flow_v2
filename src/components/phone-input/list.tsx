...
import { CountryCode, CountryListProps } from './types';
...
export function CountryListPopover({ countryCode, onClickCountry }: CountryListProps) {
  ...
  const renderButton = (
    <ButtonBase disableRipple onClick={popover.onOpen}>
      <CountryFlag code={selectedCountry.code?.toLowerCase()} />
        ...
        return (
          <MenuItem
            ...
            onClick={() => {
              popover.onClose();
              setSearchCountry('');
              onClickCountry(country.code as CountryCode);
            }}
          >
            <CountryFlag code={country.code?.toLowerCase()} sx={{ mr: 1 }} />
