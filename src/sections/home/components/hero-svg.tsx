...
type DotProps = BoxProps &
  MotionProps & {
    slotProps?: {
      dot?: MotionProps & BoxProps & { color?: ColorSchema };
    };
  };

function Dot({ color = 'primary', slotProps, sx, ...other }: DotProps) {
  ...
  return (
    ...
    >
      <Box
        component={m.div}
        animate={slotProps?.dot?.animate}
        transition={
          slotProps?.dot?.transition ?? {
            duration: 6,
            ease: 'linear',
            repeat: Infinity,
            repeatType: 'reverse',
        ...
        sx={{
          ...
          height: 1,
          borderRadius: '50%',
          boxShadow: `0px -2px 4px 0px ${theme.palette[color as ColorSchema][theme.palette.mode === 'light' ? 'main' : 'dark']} inset`,
          background: `linear-gradient(135deg, ${theme.palette[color as ColorSchema].lighter}, ${theme.palette[color as ColorSchema].light})`,
          ...slotProps?.dot?.sx,
        }}
      />
    </Box>
