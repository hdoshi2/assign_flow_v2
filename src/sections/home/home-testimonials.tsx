...
import { varFade, MotionViewport, AnimatedCountUp } from 'src/components/animate';
import Carousel, {
  useCarousel,
  CarouselDotButtons,
...
export default function HomeTestimonials({ sx, ...other }: StackProps) {
  ...
  const horizontalDivider = (position: 'top' | 'bottom') => (
    <Divider
      ...
      sx={{
        ...
        border: 'none',
        position: 'absolute',
        background: `linear-gradient(to right, ${alpha(theme.palette.grey[500], 0)} 0%, ${theme.palette.grey[500]} 50%, ${alpha(theme.palette.grey[500], 0)} 100%)`,
        ...(position === 'top' && { top: 0 }),
  ...
  const verticalDivider = (
    <Divider
      ...
      sx={{
        ...
        border: 'none',
        width: '1px',
        background: `linear-gradient(to bottom, ${alpha(theme.palette.grey[500], 0)} 0%, ${theme.palette.grey[500]} 50%, ${alpha(theme.palette.grey[500], 0)} 100%)`,
      ...
      <Carousel carousel={carousel}>
        {TESTIMONIALS.map((item) => (
          <Stack key={item.id} component={m.div} variants={varFade().in}>
            ...
            <Typography
              sx={{
                ...theme.mixins.maxLine(4, theme.typography.body1),
                mt: 2,
                mb: 3,
              }}
            >
